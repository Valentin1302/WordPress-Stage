...
class arpricelite {
		...
		if ( $pagenow == 'plugins.php' ) {
			...
			$html .= '<div class="arplite-deactivate-form-footer"><p>';
				...
				$html .= '<a id="arplite-deactivate-submit-form" class="button button-primary" href="#">'
					. sprintf( __( '%1$s Submit and%2$s Deactivate', 'arprice-responsive-pricing-table' ), '<span>', '</span>' )
					. '</a>';
	...
	function upgrade_data() {
		...
		if ( version_compare( $checkupdate, '3.6.2', '<' ) ) {
	...
	function arpsubscribeuser() {
		...
		if ( empty( $_POST['cust_email'] ) || sanitize_email( $_POST['cust_email'] ) == '' ) {
			echo 'Invalid Email';
			exit;
		}
	...
	function generateplugincode() {
		...
		$siteinfo[] = sanitize_text_field( $_SERVER['SERVER_ADDR'] );
